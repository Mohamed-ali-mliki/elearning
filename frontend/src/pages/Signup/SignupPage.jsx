    ...
    <div className="container d-flex align-items-center justify-content-center py-5">
      <div className="card shadow-lg border-0 rounded-4 p-4 p-md-5" style={{ maxWidth: '550px', width: '100%' }}>
        <div className="text-center mb-4">
          <h2 className="fw-bold">{t('auth.signupTitle')}</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="mb-3">
            ...
            <input type="text" className={`form-control ${errors.fullName ? 'is-invalid' : ''}`} placeholder="Jean Dupont"
              {...register('fullName', { required: t('auth.fullNameRequired'), minLength: { value: 2, message: t('auth.fullNameMin') } })} />
          ...
          <div className="mb-3">
            ...
            <div className="input-group">
              <input type={showConfirmPassword ? 'text' : 'password'} className={`form-control ${errors.confirmPassword ? 'is-invalid' : ''}`} placeholder={t('auth.confirmPasswordPlaceholder')}
                {...register('confirmPassword', { required: t('auth.confirmPasswordRequired'), validate: (value) => value === password || t('auth.passwordMismatch') })} />
          ...
          <div className="mb-4 form-check">
            ...
            <label className="form-check-label" htmlFor="terms">{t('auth.termsAccept')} <Link to="/terms">{t('auth.termsLink')}</Link></label>
            {errors.terms && <div className="invalid-feedback d-block">{errors.terms.message}</div>}
          </div>

          <button type="submit" className="btn btn-primary w-100 py-2 fw-semibold rounded-pill">{t('auth.signupBtn')}</button>
        ...
        <p className="text-center mt-4 mb-0">
          {t('auth.alreadyAccount')} <Link to="/login" className="text-decoration-none fw-bold">{t('auth.loginBtn')}</Link>
        </p>
      </div>
    </div>
  );
